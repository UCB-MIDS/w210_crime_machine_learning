...
#model.add(Dense(size_input*4, kernel_initializer='normal', activation='relu'))
#model.add(Dense(1, kernel_initializer='normal'))
#model.compile(loss='mean_squared_error', optimizer='adam', metrics=['mean_absolute_error'])
#checkpoint_name = 'bestweights.hdf5'
#checkpoint = ModelCheckpoint(checkpoint_name, monitor='val_loss', verbose = 1, save_best_only = True, mode ='auto')
#earlystop = EarlyStopping(monitor='val_loss', min_delta=0.01, patience=20, verbose=1, mode='auto', restore_best_weights=True)
#callbacks_list = [checkpoint]
# model.fit(df_X, df_Y, epochs=500, batch_size=32, validation_split = 0.2, callbacks=callbacks_list)
#model.fit(df_X, df_Y, epochs=1000, batch_size=16384, validation_split = 0.2, callbacks=callbacks_list, verbose=2)
